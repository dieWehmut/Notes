<template>
  <footer class="site-footer">
    <div class="survival-time">
      Uptime:<span class="time-number">{{ days }}</span>d
      <span class="time-number">{{ hours }}</span>h
      <span class="time-number">{{ minutes }}</span>m 
      <span class="time-number">{{ seconds }}</span>s🕒
    </div>
...
export default {
  ...
  mounted() {
    ...
    setInterval(this.updateSurvivalTime, 1000);
  },
  methods: {
    updateSurvivalTime() {
      const now = new Date();
